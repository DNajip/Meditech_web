...
public class PacientesController(MediTechContext context) : Controller
{
    ...
    // GET: /Pacientes/GetResumen/5
    [HttpGet]
    public async Task<IActionResult> GetResumen(int id)
    {
        try
        {
            // 1. Alertas de Historial
            var historial = await _context.HistorialesClinicos
                .AsNoTracking()
                .FirstOrDefaultAsync(h => h.IdPaciente == id);

            // 2. Última Consulta y Signos Vitales
            var ultimaConsulta = await _context.Consultas
                .AsNoTracking()
                .Include(c => c.SignosVitales)
                .Include(c => c.Cita)
                    .ThenInclude(ci => ci!.Tratamiento)
                .Where(c => c.Cita != null && c.Cita.IdPaciente == id && c.IdEstado != 0) 
            ...
            var proximaCita = await _context.Citas
                ...
                .Where(c => c.IdPaciente == id && c.Fecha >= hoy.Date && c.IdEstadoCita == 1) // 1=PENDIENTE
                .OrderBy(c => c.Fecha)
                .ThenBy(c => c.HoraInicio)
                .FirstOrDefaultAsync();

            // 4. Últimas 3 Fotos
            var fotosDb = await _context.FotosClinicas
                .AsNoTracking()
            ...
            // Transformar en memoria para evitar errores de traducción LINQ (ej: ToString con formato)
            var fotos = fotosDb.Select(f => new { 
                f.IdFoto, 
                f.Titulo, 
                fechaRes = f.FechaRegistro.ToString("dd/MM/yyyy") 
            }).ToList();

            return Json(new {
                success = true,
                alertas = historial != null ? new {
                    alergias = historial.Alergias,
                    alergiasDetalle = historial.AlergiasDetalle,
                    diabetes = historial.Diabetes,
                    hipertension = historial.Hipertension,
                    tomaMedicamento = historial.TomaMedicamento,
                    medicamentoDetalle = historial.MedicamentoDetalle,
                    embarazada = historial.Embarazada
                } : null,
                ultimaConsulta = ultimaConsulta != null ? new {
                    fecha = ultimaConsulta.FechaConsulta.ToString("dd/MM/yyyy"),
                    motivo = ultimaConsulta.MotivoConsulta,
                    diagnostico = ultimaConsulta.DiagnosticoPrincipal,
                    tratamiento = ultimaConsulta.Cita?.Tratamiento?.NombreTratamiento,
                    signos = ultimaConsulta.SignosVitales != null ? new {
                        pa = ultimaConsulta.SignosVitales.PresionArterial,
                        fc = ultimaConsulta.SignosVitales.FrecuenciaCardiaca,
                        temp = ultimaConsulta.SignosVitales.Temperatura,
                        bmi = ultimaConsulta.SignosVitales.BMI
                    } : null
                } : null,
                proximaCita = proximaCita != null ? new {
                    fecha = proximaCita.Fecha.ToString("dd/MM/yyyy"),
                    hora = proximaCita.HoraInicio.ToString(@"hh\:mm"),
                    tratamiento = proximaCita.Tratamiento?.NombreTratamiento
                } : null,
                fotos = fotos
            });
        }
        ...
        {
            // Log the error (if possible, here we just return it)
